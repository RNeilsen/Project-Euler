...
def main():
    ...
    while target not in visitednodes:
        ...
        for curnode in visitednodes:
            ...
            if x > 0 and (y,x-1) not in visitednodes:  # left neighbour has not yet been visited
                if visitednodes[curnode] + m[y][x-1] < bestsum:  # path to left neighbour has lowest known sum
                    bestsum = visitednodes[curnode] + m[y][x-1]
                    bestxy = (y, x-1)

            if x < width - 1 and (y,x+1) not in visitednodes:  # right neighbour has not yet been visited
                if visitednodes[curnode] + m[y][x+1] < bestsum:  # path to right neighbour has lowest known sum
                    bestsum = visitednodes[curnode] + m[y][x+1]
                    bestxy = (y, x+1)

            if y > 0 and (y-1,x) not in visitednodes:  # upper neighbour has not been visited
                if visitednodes[curnode] + m[y-1][x] < bestsum:  # path to upper neighbour has lowest known sum
                    bestsum = visitednodes[curnode] + m[y-1][x]
                    bestxy = (y-1, x)

            if y < height - 1 and (y+1,x) not in visitednodes:  # lower neighbour has not been visited
                if visitednodes[curnode] + m[y+1][x] < bestsum:  # path to lower neighbour has lowest known sum
                    bestsum = visitednodes[curnode] + m[y+1][x]
                    bestxy = (y+1, x)
